import {
  Injectable,
  Logger,
  OnModuleDestroy,
  OnModuleInit,
} from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { PrismaPg } from '@prisma/adapter-pg';
import { PrismaClient } from '../../generated/prisma/client';

/**
 * Mở một kết nối tới Supabase tốn ~300–500 ms (bắt tay TLS + pooler), trong khi
 * một truy vấn trên kết nối sẵn có chỉ tốn một vòng mạng (~55 ms từ VN, ~200 ms
 * từ Render/Oregon).
 *
 * Mặc định của `pg` là đóng kết nối nhàn rỗi sau **10 giây**. Với một công cụ cá
 * nhân lưu lượng thấp thì gần như *mọi* request đều rơi vào sau khoảng lặng >10s
 * — tức là gần như mọi request đều phải trả thêm nguyên chi phí mở kết nối.
 *
 * Đo thật: sau 35 giây nhàn rỗi, một `SELECT 1` tốn **350 ms** với mặc định, và
 * **54 ms** khi giữ kết nối. Đó là lý do đặt `idleTimeoutMillis: 0`.
 */
const POOL_OPTIONS = {
  /** 0 = không bao giờ đóng kết nối nhàn rỗi. */
  idleTimeoutMillis: 0,
  /**
   * Giữ sẵn hai kết nối để request đầu tiên sau khi khởi động (và nhánh chạy
   * song song đầu tiên) không phải bắt tay lại từ đầu.
   */
  min: 2,
  /**
   * Supabase pooler có trần kết nối cho cả dự án. 10 là mặc định của `pg` và đã
   * thừa cho một tiến trình Node đơn lẻ; đặt tường minh để trần không đổi theo
   * phiên bản thư viện.
   */
  max: 10,
  /** TCP keep-alive: giữ đường truyền sống qua NAT/idle timeout của hạ tầng. */
  keepAlive: true,
};

@Injectable()
export class PrismaService
  extends PrismaClient
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(PrismaService.name);

  constructor(configService: ConfigService) {
    super({
      adapter: new PrismaPg({
        connectionString: configService.get<string>('database.url'),
        ...POOL_OPTIONS,
      }),
      // Thời gian phản hồi của API gần như hoàn toàn là `số truy vấn tuần tự ×
      // độ trễ mạng`, nên "endpoint này chạy bao nhiêu truy vấn" là con số phải
      // đếm được khi có nghi ngờ. Tắt mặc định; bật bằng PRISMA_LOG_QUERIES=true.
      log: process.env.PRISMA_LOG_QUERIES === 'true' ? ['query'] : [],
    });
  }

  async onModuleInit(): Promise<void> {
    await this.$connect();

    // `$connect()` chưa thật sự mở kết nối tới Postgres — nó chỉ dựng client.
    // Một truy vấn rỗng lúc khởi động đẩy chi phí bắt tay sang deploy thay vì
    // sang người dùng đầu tiên.
    try {
      await this.$queryRaw`SELECT 1`;
    } catch (error) {
      this.logger.warn(
        `Không làm nóng được kết nối DB lúc khởi động: ${(error as Error).message}`,
      );
    }
  }

  async onModuleDestroy(): Promise<void> {
    await this.$disconnect();
  }
}
